...
class TFObjDetEval:
    def __init__(self, model):
        ...
        #     model_cfg['scales'],
        #     model_cfg['feature_map_sizes'],
        #     model_cfg['aspect_ratios']
        # )
    ...
    def eval(self,
        ...
        for batch_image_ids, batch_image_shapes, batch_images, _, _ in dataset:
            ...
            for image, image_id, image_shape, confs, locs in zip(
                ...
                print(data_loader.num_classes, batch_image_ids.shape)
                for c in range(1, data_loader.num_classes):
                    ...
                    nms_idx = compute_nms(cls_boxes, cls_scores, nms_threshold, max_predictions)
                ...
                boxes_resized = boxes * np.array([[*data_loader.img_size*2]]).astype(np.float32)
                ...
                if draw_predictions:
                    dest_path = os.path.join(image_out_dir, image_id_str+'.jpg')
                    image *= 255
                    pred_bbox_image = draw_bboxes(image[...,::-1].numpy(), boxes_resized, classes, scores, class_map)
