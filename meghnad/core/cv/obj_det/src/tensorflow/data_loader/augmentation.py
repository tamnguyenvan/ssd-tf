...
def buil_augmentation(cfg: Dict, augmentations: Dict, phase: str = 'train'):
    assert phase in ['train', 'val', 'test']

    transforms = dict()
    if not augmentations:
        augmentations_cfg = cfg.get('augmentations')
        if augmentations and augmentations.get(phase):
            transforms = augmentations_cfg[phase]
    elif augmentations.get(phase):
        transforms = augmentations[phase]

    print(phase, transforms, augmentations_cfg)

    def augment(img: tf.Tensor, gt_boxes: tf.Tensor, gt_labels: tf.Tensor):
        for aug, aug_params in transforms.items():
            if aug in AUGMENTATIONS:
                img, gt_boxes = globals()[aug](img, gt_boxes, **aug_params)
        return img, gt_boxes, gt_labels
    return augment


def apply(img: tf.Tensor, gt_boxes: tf.Tensor, gt_labels: tf.Tensor):
    ...
    color_methods = [random_brightness,
    ...
    for augmentation_method in geometric_methods + color_methods:
        img, gt_boxes = randomly_apply_operation(
            augmentation_method, img, gt_boxes)
    #
...
def get_random_min_overlap():
    ...
    i = tf.random.uniform((), minval=0, maxval=tf.shape(
        overlaps)[0], dtype=tf.int32)
    return overlaps[i]
...
def expand_image(img, gt_boxes, height, width):
    ...
    """
    expansion_ratio = tf.random.uniform(
        (), minval=1, maxval=4, dtype=tf.float32)
    final_height, final_width = tf.round(
        height * expansion_ratio), tf.round(width * expansion_ratio)
    pad_left = tf.round(tf.random.uniform(
        (), minval=0, maxval=final_width - width, dtype=tf.float32))
    pad_top = tf.round(tf.random.uniform(
        (), minval=0, maxval=final_height - height, dtype=tf.float32))
    ...
    expanded_image = tf.pad(
        img, ((pad_top, pad_bottom), (pad_left, pad_right), (0, 0)), constant_values=-1)
    expanded_image = tf.where(expanded_image == -1, mean, expanded_image)
    #
    min_max = tf.stack([-pad_top, -pad_left, pad_bottom + height,
                       pad_right + width], -1) / [height, width, height, width]
    modified_gt_boxes = bbox_utils.renormalize_bboxes_with_min_max(
        gt_boxes, min_max)
    #
...
def patch(img, gt_boxes):
    ...
    img, gt_boxes = randomly_apply_operation(
        expand_image, img, gt_boxes, org_height, org_width)
    ...
    gt_boxes = bbox_utils.renormalize_bboxes_with_min_max(
        gt_boxes, new_boundaries[0, 0])
    #
    return img, gt_boxes
